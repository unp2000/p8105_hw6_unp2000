...
low birthweight. Based on a study conducted by Marieke Paarlberg et al.,
2005, Baby Sex (babysex), Gestational Age in Weeks (gaweeks), Mother’s
Age at Delivery (momage), Mother’s Height (mheight), Parity (parity),
Mother’s Pre-Pregnancy BMI (ppbmi), Previous Number of Low Birthweight
Babies (pnumlbw), and Average Number of Cigarettes Smoked per day During
Pregnancy (smoken) were selected as variables to include in the
model.
...
```
    ...
    ##  1 <res… <res… 001   <lm>      <lm>      <lm>             443.        314.
    ##  2 <res… <res… 002   <lm>      <lm>      <lm>             455.        333.
    ##  3 <res… <res… 003   <lm>      <lm>      <lm>             461.        334.
    ##  4 <res… <res… 004   <lm>      <lm>      <lm>             442.        315.
    ##  5 <res… <res… 005   <lm>      <lm>      <lm>             433.        333.
    ##  6 <res… <res… 006   <lm>      <lm>      <lm>             456.        334.
    ##  7 <res… <res… 007   <lm>      <lm>      <lm>             470.        321.
    ##  8 <res… <res… 008   <lm>      <lm>      <lm>             450.        351.
    ##  9 <res… <res… 009   <lm>      <lm>      <lm>             454.        331.
    ## 10 <res… <res… 010   <lm>      <lm>      <lm>             469.        320.
    ## # … with 90 more rows, and 1 more variable: rmse_model3 <dbl>

Based off of the RMSE distribution of the models, I can see that the
Model 3 which includes the interaction terms is the best fit for this
data. Model 1, my predicted model based on the literature has a high
RMSE value which indicates that it doesn’t fit the data appropriately.
Based off the cross validation prediction error which gives us RMSE
values, Model 2 which includes some of the parameters from Model 3 has
an RMSE value in the middle which shows that it fits the data better
than Model 1 but not as well as Model 3.
...
![](p8105_hw6_unp2000_files/figure-gfm/unnamed-chunk-12-1.png)<!-- -->

Based on the plot above, we can see that the distribution of log(b0 \*
b1) is approximately normal which might be due to the fact that we
performed a log transformation to log(b0 \* b1).We can infer that 95%
confidence interval for log(b0\*b1) based off the 2.5% and 97.5%
quantiles is approximately (1.925, 2.075).
...
![](p8105_hw6_unp2000_files/figure-gfm/unnamed-chunk-13-1.png)<!-- -->

Based on the plot above, we can see that the distribution for our r^2
values is slightly left skewed. We can infer that the 95% confidence
interval for r^2 based off the 2.5% and 97.5% quantiles is approximately
(0.8875, 0.9375).
